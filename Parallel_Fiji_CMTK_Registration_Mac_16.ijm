...
function warp(warp_list,affine_list,X,C,R,G,T,warp_accuracy,speed,warp_reg_metric,final_res) {	
	warp_command = "/opt/local/lib/cmtk/bin/warp "+warp_reg_metric+" --threads "+T+" --jacobian-weight 0 "+speed+" -e "+X+" --grid-spacing "+G+" --energy-weight 1e-1 --refine "+R+" --coarsest "+C+" --ic-weight 0 --output-intermediate --accuracy "+warp_accuracy+" "+final_res+" -o "+warp_list+" "+affine_list+";";
	return warp_command;
	}
