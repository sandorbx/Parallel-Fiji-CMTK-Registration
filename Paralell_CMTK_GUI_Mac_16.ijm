...
function reformatx(reg_folder_path,refbrain_path,reformat_list, transf_list) {
	...
	for (i = 0; i < reformat_list.length; i++) {
		...
		if (operation == "Affine Registration"){
			command = "/opt/local/lib/cmtk/bin/reformatx --pad-out 0 -o "+reg_folder_path+"/Reformatted/affine_"+outfilename+".nrrd --floating "+reformat_list[i]+" "+refbrain_path+" "+transf_list+" & ";
			}   
		else {			
			command = "/opt/local/lib/cmtk/bin/reformatx --pad-out 0 -o "+reg_folder_path+"/Reformatted/warp_"+outfilename+".nrrd --floating "+reformat_list[i]+" "+refbrain_path+" "+transf_list+" & ";
			}
...
function affine(refbrain_path,registration_channel_path,affine_list,dof1,dof2,affine_accuracy,init_mode,init_list,affine_X,affine_reg_metric,final_res) {
	affine_command = "/opt/local/lib/cmtk/bin/make_initial_affine "+init_mode+" "+refbrain_path+" "+registration_channel_path+" "+init_list+"; /opt/local/lib/cmtk/bin/registration --initial "+init_list+" "+affine_reg_metric+" --dofs "+dof1+" --dofs "+dof2+" "+affine_reg_metric+" --exploration "+affine_X+" --accuracy "+affine_accuracy+" "+final_res+" -o "+affine_list+" "+refbrain_path+" "+registration_channel_path+"; ";
	return affine_command;
	}

function warp(warp_list,affine_list,X,C,R,G,T,warp_accuracy,speed,warp_reg_metric,final_res) {	
	warp_command = "/opt/local/lib/cmtk/bin/warp "+warp_reg_metric+" --threads "+T+" --jacobian-weight 0 "+speed+" -e 18 --grid-spacing "+G+" --energy-weight 1e-1 --refine "+R+" --coarsest "+C+" --ic-weight 0 --output-intermediate --accuracy "+warp_accuracy+" "+final_res+" -o "+warp_list+" "+affine_list+";";
	return warp_command;
	}
...
for (j = 0; j < directory_path_list.length; j++) {
	...
	for (i = 0; i < imagefile_list.length; i++) {
		if (matches(imagefile_list[i], ".*_01.*")==1) {
			...
			init_list = reg_folder_path+"/"+name+"_"+"init.list";
			...
			if (operation == "Affine Registration") {
				affine_list = reg_folder_path+"/"+name+"_"+"affine.list";
			...
			if (operation == "Affine and Warp Registration") {
				affine_list = reg_folder_path+"/"+name+"_"+"affine.list";
				affine_check_list = Array.concat(affine_check_list,affine_list);
				warp_list = reg_folder_path+"/"+name+"_"+"warp.list";
